...
class Model(nn.Module):
    ...
    def forward_logits(self, input_ids, attention_mask, aug=False, save_pred_seq=False, return_embed=False):
        ...
        if self.msd=='true' and self.training:
            ...
            logits1 = self.head(logits) # (bs,num_tokens,num_labels)
    ...
    def training_step(self, batch):
        ...
        if self.loss in ['mse','l1','smoothl1']:
            if self.mt=='true' and self.training:
                ...
                mask = (input_data['attention_mask']==1)
                active_logits = logits[mask]
                active_label = data['label'][mask]
                loss = self.get_losses(active_logits,active_label).mean()
        else:
            raise NotImplementedError()
        
        mask = (input_data['attention_mask']==1).unsqueeze(2)
        logits = (logits*mask).sum(dim=1) / mask.sum(dim=1)
        
        pred = logits.detach().cpu().numpy() # (bs,num_labels)
        label = (data['label']*mask).sum(dim=1) / mask.sum(dim=1)
        label = label.detach().cpu().numpy() # (bs,num_labels)
    ...
    def validation_step(self, batch):
        ...
        if self.loss in ['mse','l1','smoothl1']:
            ...
            active_logits = logits[mask]
            active_label = data['label'][mask]
            loss = self.get_losses(active_logits,active_label).mean().detach().cpu().numpy()
        else:
            raise NotImplementedError()
        ...
        output = {
            #'loss':loss,
            'pred':pred,
            'label':data['label'].detach().cpu().numpy()[:,0],
            'text':data['text'],
...
class CustomCollator(object):
    ...
    def __call__(self, samples):
        ...
        if 'label' in output.keys():
            bs = len(output['label'])
            label = torch.FloatTensor(output['label']).reshape(bs,1,len(self.target_cols))
            mask = output["attention_mask"].float()
            mask = mask.unsqueeze(2).expand(bs,batch_max,len(self.target_cols))
            output['label'] =  label * mask
        return output