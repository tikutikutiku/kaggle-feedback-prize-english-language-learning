...
def run(args, trn_df, val_df, pseudo_df=None):
    ...
    if True:
        ...
        if 'tascj' in args.pretrained_detector_path:
            ...
            model_pretraining.load_state_dict(torch.load(args.pretrained_detector_path))
        elif args.pretrained_effectiveness_path != 'none':
            discourse_type_list = [
                'Lead',
                'Position',
                'Claim',
            ...
            if 'deberta-v2' in args.model or 'deberta-v3' in args.model:
                from transformers.models.deberta_v2 import DebertaV2TokenizerFast
                pre_tokenizer = DebertaV2TokenizerFast.from_pretrained(args.model, trim_offsets=False)
                pre_special_tokens_dict = {'additional_special_tokens': ['\n\n'] + [f'[{s.upper()}]' for s in discourse_type_list]}
                _ = pre_tokenizer.add_special_tokens(pre_special_tokens_dict)
            else:
                pre_tokenizer = AutoTokenizer.from_pretrained(args.model, trim_offsets=False)
                pre_special_tokens_dict = {'additional_special_tokens': [f'[{s.upper()}]' for s in discourse_type_list]}
                _ = pre_tokenizer.add_special_tokens(pre_special_tokens_dict)
            from models_effectiveness import Model as EffectivenessModel
            model_pretraining = EffectivenessModel(
                args.model,
                pre_tokenizer,
                num_labels=3,
                num_labels_2=-1,
            ...
            model_pretraining.load_state_dict(torch.load(args.pretrained_effectiveness_path))
            ...
            if 'deberta-v2' in args.model or 'deberta-v3' in args.model:
                ...
                pre_special_tokens_dict = {'additional_special_tokens': [f'[{s.upper()}]' for s in discourse_type_list]}
                _ = pre_tokenizer.add_special_tokens(pre_special_tokens_dict)
            from models_mlm import Model as MLMModel
            model_pretraining = MLMModel(
                args.model,
                pre_tokenizer,
                num_labels=len(pre_tokenizer),
            )
