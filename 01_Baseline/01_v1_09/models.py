...
class Model(nn.Module):
    ...
    def forward_logits(self, input_ids, attention_mask, aug=False, save_pred_seq=False):
        ...
        # pooling
        logits = []
        for _hidden_states,_attention_mask in zip(hidden_states, attention_mask):
            logits.append(_hidden_states[:sum(_attention_mask==1)].mean(0))
        logits = torch.stack(logits) # (bs, hidden_size*multi_layers) 
