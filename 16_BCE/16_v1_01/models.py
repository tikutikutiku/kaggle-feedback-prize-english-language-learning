...
class Model(nn.Module):
    def __init__(self, 
        ...
        if freeze_layers == 'true':
            ...
            elif 'deberta-v2-xlarge' in model_name or 'deberta-xlarge' in model_name:
                print('freeze 12/24')
                self.transformer.embeddings.requires_grad_(False)
                self.transformer.encoder.layer[:12].requires_grad_(False) # freeze 12/24
            elif 'deberta-v3-large' in model_name:
                print('freeze 12/24')
                self.transformer.embeddings.requires_grad_(False)
                self.transformer.encoder.layer[:12].requires_grad_(False) # freeze 12/24
        ...
        if loss=='mse':
            ...
            self.eye = torch.eye(self.num_labels).cuda()
            self.coeff_arr = np.array([1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]).reshape(-1,self.num_labels)
        elif loss=='xentropy':
            self.loss_fn = nn.CrossEntropyLoss(reduction='none')
            self.coeff_arr = np.array([1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5])
        else:
            raise Exception()
    ...
    def training_step(self, batch):
        ...
        if self.loss in ['mse','l1','smoothl1']:
            ...
            loss = self.get_losses(
                logits.reshape(-1,self.num_labels),
                self.eye[data['label'].reshape(-1,).long()]
            ).mean()
        elif self.loss in ['xentropy']:
            logits = self.forward_logits(**input_data)
            loss = self.get_losses(
                logits.reshape(-1,self.num_labels),
                data['label'].reshape(-1,).long()
            ).mean()
        else:
            raise NotImplementedError()

        if self.loss in ['bce']:
            pred = (logits.softmax(-1).detach().cpu().numpy() * self.coeff_arr).sum(-1)
            pred = pred.reshape(-1,1)
            label = data['label'].detach().cpu().numpy().reshape(-1,1)
        elif self.loss in ['xentropy']:
            #pred = self.coeff_arr[logits.argmax(-1).detach().cpu().numpy()]
            pred = (logits.softmax(-1).detach().cpu().numpy() * self.coeff_arr).sum(-1)
            pred = pred.reshape(-1,1)
            label = data['label'].detach().cpu().numpy().reshape(-1,1)
        else:
