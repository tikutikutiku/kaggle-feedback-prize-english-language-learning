...
class WeightedLayerLSTMPooling(nn.Module):
    def __init__(self, num_layers, hidden_dim):
        super().__init__()
        self.num_layers = num_layers
        self.layer_weights = nn.Parameter(torch.tensor([1] * num_layers, dtype=torch.float)) # (num_layers,)
        self.lstm = ResidualLSTM(hidden_dim, rnn='LSTM')
...
class Model(nn.Module):
    def __init__(self, 
        ...
        if last_layer_reinit=='true':
            print("re-initialize last layer...")
            last_layer = self.transformer.encoder.layer[-1]
            for module in last_layer.modules():
        ...
        if use_separate_head=='true':
            if self.pooling=='attention':
                ...
                self.pooling_fn = nn.ModuleList([
                    WeightedLayerPooling(num_pooling_layers)
                    for _ in range(num_labels)
                ])
            elif self.pooling=='weighted_layer_lstm':
                self.pooling_fn = nn.ModuleList([
                    WeightedLayerLSTMPooling(num_pooling_layers, self.config.hidden_size*self.multi_layers + num_feats)
                    for _ in range(num_labels)
                ])
            else:
                raise Exception()
            ...
            if self.pooling=='mean':
                ...
                self.pooling_fn = WeightedLayerLSTMPooling(num_pooling_layers, self.config.hidden_size*self.multi_layers + num_feats)
...
#             self._init_weights(self.head2)
    ...
    def forward_logits(self, input_ids, attention_mask, aug=False, save_pred_seq=False, return_embed=False, feats=None):
        ...
        if self.window_size==-1 or L <= self.window_size:
            ...
            elif self.pooling in ['weighted_layer', 'weighted_layer_lstm']:
...
#             logits = lam * logits + (1 - lam) * logits[index, :]
    ...
    def fetch_optimizer(self):
        ...
        if self.pooling in ['weighted_layer_lstm', 'weighted_layer','attention','transformer+mean']:
        ...
        if self.layerwise_lr_decay != -1:
            print("setting layerwise lr decay...")
