...
def run(args, trn_df, val_df, pseudo_df=None, feature_cols=None):
    ...
    if True:
        ...
        if args.use_stats=='true':
            trn_features = trn_df[feature_cols].values
        ...
        trn_dataset = DatasetTrain(
            ...
            target_cols=target_cols,
            features=trn_features if feature_cols else None,
        )
        ...
        model = Model(args.model, 
                      ...
                      last_layer_reinit=args.last_layer_reinit,
                            ...
                            if args.class_name!='none':
                                if args.mode=='pseudo':
                                    torch.save(model.state_dict(), opj(
                                        output_path,f'model_{args.class_name}_seed{args.seed}_fold{args.fold}_pseudo.pth'))
                                else:
                                    torch.save(model.state_dict(), opj(
                                        output_path,f'model_{args.class_name}_seed{args.seed}_fold{args.fold}.pth'))
                                print(f'model_{args.class_name} (best score) saved')
                            else:
                                if args.mode=='pseudo':
                                    torch.save(model.state_dict(), opj(
                                        output_path,f'model_seed{args.seed}_fold{args.fold}_pseudo.pth'))
                                else:
                                    torch.save(model.state_dict(), opj(
                                        output_path,f'model_seed{args.seed}_fold{args.fold}.pth'))
                                print(f'model (best score) saved')
