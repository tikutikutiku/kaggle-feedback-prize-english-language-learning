...
class Model(nn.Module):
    ...
    def forward_logits(self, input_ids, attention_mask, aug=False, save_pred_seq=False, return_embed=False):
        ...
        bias = self.bias_head(logits)
            
        if return_embed:
            if save_pred_seq:
                return (bias,logits1), pred_seq, embed
            else:
                return (bias,logits1), embed
        else:
            if save_pred_seq:
                return (bias,logits1), pred_seq
            else:
                return (bias,logits1)
    ...
    def training_step(self, batch):
        ...
        if self.loss in ['mse','l1','smoothl1']:
            (bias,logits) = self.forward_logits(**input_data)
            loss = self.get_losses(bias.reshape(-1,1), data['label_avg'].reshape(-1,1)).mean()
            loss += self.get_losses(logits.reshape(-1,self.num_labels), data['label_diff'].reshape(-1,self.num_labels)).mean()
        else:
            raise NotImplementedError()

        pred = (bias.reshape(-1,1) + logits).detach().cpu().numpy() # (bs,num_labels)
    ...
    def validation_step(self, batch):
        ...
        if self.loss in ['mse','l1','smoothl1']:
            ...
            (bias, logits), pred_seq, embed = self.forward_logits(**input_data)
            loss = self.get_losses(bias.reshape(-1,1), data['label_avg'].reshape(-1,1)).detach().cpu().numpy()
            loss = loss + self.get_losses(logits.reshape(-1,self.num_labels), data['label_diff'].reshape(-1,self.num_labels)).detach().cpu().numpy()
        else:
            raise NotImplementedError()
            
        # get pred
        pred = (bias.reshape(-1,1) + logits).detach().cpu().numpy().reshape(-1,self.num_labels)
    ...
    def fetch_optimizer(self):
        head_params = list(self.head.named_parameters()) + list(self.bias_head.named_parameters())
        if self.rnn!='none':
            head_params += list(self.rnn_head.named_parameters())
        if self.pooling in ['weighted_layer']:
...
class DatasetTrain(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels_avg = sample_df['avg'].values[0]
        labels_diff = sample_df[[f'diff_{col}' for col in self.target_cols]].values[0]
        ...
        output = dict(
            ...
            attention_mask = attention_mask,
            label = labels,
            label_avg = labels_avg,
            label_diff = labels_diff,
        )
        return output
...
class CustomCollator(object):
    ...
    def __call__(self, samples):
        ...
        if 'label_avg' in output.keys():
            output['label_avg'] = torch.FloatTensor(output['label_avg'])
            
        if 'label_diff' in output.keys():
            output['label_diff'] = torch.FloatTensor(output['label_diff'])
