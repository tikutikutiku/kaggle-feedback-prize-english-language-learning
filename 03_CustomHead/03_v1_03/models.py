...
#             )
            self.head = nn.ModuleList([
                nn.Sequential(
                    AttentionPooling(self.config.hidden_size*self.multi_layers),
...
#             logits1 = self.head(logits) # (bs,num_labels)
        ...
        logits1 = []
        for i in range(len(TARGET_COLS)):
            logits1.append(self.head[i](logits))
        logits1 = torch.cat(logits1, dim=1) # (bs,num_labels)
