...
def parse_args():
    ...
    parser.add_argument("--fb3_only", type=str, default='false', required=False)
...
if __name__=='__main__':
    ...
    if args.fb3_only=='true':
        fb1_train_df = pd.read_csv('../../input/feedback-prize-2021/train.csv')
        trn_df = trn_df[~trn_df['essay_id'].isin(fb1_train_df['id'])].reset_index(drop=True)
        print('trn_df.shape = ', trn_df.shape)
    elif args.fb3_only=='trn+val':
        fb1_train_df = pd.read_csv('../../input/feedback-prize-2021/train.csv')
        trn_df = trn_df[~trn_df['essay_id'].isin(fb1_train_df['id'])].reset_index(drop=True)
        val_df = val_df[~val_df['essay_id'].isin(fb1_train_df['id'])].reset_index(drop=True)
        print('trn_df.shape = ', trn_df.shape)
        print('val_df.shape = ', val_df.shape)
